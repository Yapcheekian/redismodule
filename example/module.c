...
// https://github.com/RedisLabsModules/RedisModulesSDK/blob/7ba899d7b75448e527bcb6f40b1091c5346eac8a/example/module.c#L11
...
int testParse(RedisModuleCtx *ctx) {
  ...
  RedisModuleCallReply *r =
      RedisModule_Call(ctx, "example3.parse", "ccc", "SUM", "5", "2");
  RMUtil_Assert(RedisModule_CallReplyType(r) == REDISMODULE_REPLY_INTEGER);
  RMUtil_AssertReplyEquals(r, "7");

  r = RedisModule_Call(ctx, "example3.parse", "ccc", "PROD", "5", "2");
  RMUtil_Assert(RedisModule_CallReplyType(r) == REDISMODULE_REPLY_INTEGER);
...
int testHgetSet(RedisModuleCtx *ctx) {
  RedisModuleCallReply *r =
      RedisModule_Call(ctx, "example3.hgetset", "ccc", "foo", "bar", "baz");
  RMUtil_Assert(RedisModule_CallReplyType(r) != REDISMODULE_REPLY_ERROR);

  r = RedisModule_Call(ctx, "example3.hgetset", "ccc", "foo", "bar", "bag");
  RMUtil_Assert(RedisModule_CallReplyType(r) == REDISMODULE_REPLY_STRING);
  RMUtil_AssertReplyEquals(r, "baz");
  r = RedisModule_Call(ctx, "example3.hgetset", "ccc", "foo", "bar", "bang");
...
int RedisModule_OnLoad(RedisModuleCtx *ctx) {
  ...
  if (RedisModule_Init(ctx, "example3", 1, REDISMODULE_APIVER_1) ==
      REDISMODULE_ERR) {
    return REDISMODULE_ERR;
  }

  // register example.parse - the default registration syntax
  if (RedisModule_CreateCommand(ctx, "example3.parse", ParseCommand, "readonly",
                                1, 1, 1) == REDISMODULE_ERR) {
    return REDISMODULE_ERR;
  }

  // register example.hgetset - using the shortened utility registration macro
  RMUtil_RegisterWriteCmd(ctx, "example3.hgetset", HGetSetCommand);

  // register the unit test
  RMUtil_RegisterWriteCmd(ctx, "example3.test", TestModule);
